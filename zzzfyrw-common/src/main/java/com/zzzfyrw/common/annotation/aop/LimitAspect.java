...
public class LimitAspect {
    ...
    @Autowired
    private LettuceHandler<Number> lettuceHandler;


    @Around("execution(public * *(..)) && @annotation(com.zzzfyrw.common.cache.config.RedisLimit)")
    public Object interceptor(ProceedingJoinPoint pjp) {
        ...
        switch (limitType) {
            case IP:
                ...
                key = IPv4Utils.getIpAddress(request);
                break;
            case CUSTOMER:
        ...
        try {
            ...
            Number count = lettuceHandler.execute(redisScript, keys, limitCount, limitPeriod);
            log.info("Access try count is {} for name={} and key = {}", count, name, key);
            if (count != null && count.intValue() <= limitCount) {
                return pjp.proceed();
            } else {
                throw new ZBootException("You have been dragged into the blacklist");
            }
        } catch (Throwable e) {
